...
uses
  Windows, Messages, Classes, Graphics, SysUtils, Registry, ShellAPI, IniFiles;
...
uses RegExpr, Browse4Folder, myUtils;
...
begin
  ...
  begin
    ...
    try
      Title := Format('Junction from ''?'' to ''%s''', [FJunction_To]);
...
begin
  ...
  try
    ...
    mii.dwTypeData := PChar('Junction Shell');
    mii.cch := Length('Junction Shell');
    if InsertMenuItem(hParentMenu, nPosition, True, mii) = FALSE then Exit;
    _AddMenuEnd(nCommand, nPosition, 'Junction Shell', '', '', 'junction shell', '');
    //DrawMenuBar(Handle);
    ...
    //submenu
    _AddMenuBegin(ifirstCMDID, ifirstINDEX, nCommand, nPosition);
    InsertMenu(hContextMenu, nPosition ,MF_BYPOSITION or MenuStyleEx, nCommand, 'Select...');
    _AddMenuEnd(nCommand, nPosition, 'Select...', '', '', 'select...', 'SYS_SELECT');
    ...
    begin
      ...
      if FData.IsCurrent(FJunction_Froms[i], FJunction_To) and (FData.DrawType = 0) then
        InsertMenu(hContextMenu, nPosition ,MF_BYPOSITION, nCommand, PChar('*' + FJunction_Froms[i]))
      else
        InsertMenu(hContextMenu, nPosition ,MF_BYPOSITION or MenuStyleEx, nCommand, PChar(FJunction_Froms[i]));
